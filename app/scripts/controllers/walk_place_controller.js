 ...
 */
app.controller('walkPlaceCtrl', ['rootService', '$location', 'userServiceData', 'walkService', 'walkServiceData', 'locationService', 'locationServiceData', '$routeParams', '$scope', '$timeout',
  function (rootService, $location, userServiceData, walkService, walkServiceData, locationService, locationServiceData, $routeParams, $scope, $timeout) {
    ...
    function initWatchers() {
      ...
      vm.walkWatcher = $scope.$watch(
        function () {
          return walkService.walkListByPlaceLoaded;
        }, function (newValue) {
          if (newValue === true) {
            getWalks();
            checkUserIsSubscribed();
            walkService.walkListByPlaceLoaded = false;
          }
        }
      );
      ...
      vm.participantsWatcher = $scope.$watch(
        function () {
          return walkService.placeParticipantsLoaded;
        }, function (newValue) {
          if (newValue === true) {
            getParticipantData();
            walkService.placeParticipantsLoaded = false;
          }
        }
      );
